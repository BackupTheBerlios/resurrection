	...
	if (iArgs == 2){
		...
		if (!strUnits.IsEmpty()){
			if (strUnits.CompareNoCase(_T("b")) == 0 || strUnits.CompareNoCase(_T("byte")) == 0 || strUnits.CompareNoCase(_T("bytes")) == 0)
				...
				AfxMessageBox(GetResString(IDS_SEARCH_EXPRERROR) + _T("\n\n") + GetResString(IDS_SEARCH_INVALIDMINMAX)/*, MB_ICONWARNING | MB_HELP, eMule_FAQ_Search - HID_BASE_PROMPT*/);//removed help [lama]
				return (ULONG)-1;
			}
		}
	}
...
SSearchParams* CSearchParamsWnd::GetParameters()
{
	...
	if (!IsValidEd2kString(strExpression)){
		AfxMessageBox(GetResString(IDS_SEARCH_EXPRERROR) + _T("\n\n") + GetResString(IDS_SEARCH_INVALIDCHAR)/*, MB_ICONWARNING | MB_HELP, eMule_FAQ_Search - HID_BASE_PROMPT*/);//removed help [lama]
		return NULL;
	}
