...
protected:
	...
	virtual BOOL MakeMimeHeader(CStringA& header, LPCSTR szBoundary, LPCTSTR szFileName) throw()
	{
		...
		if (*szBoundary)
		{
			...
			MEMCOPY(szBegin, "\r\n\r\n--", 6);
			MEMCOPY(szBegin+6, szBoundary, ATL_MIME_BOUNDARYLEN);
			*(szBegin+(ATL_MIME_BOUNDARYLEN+6)) = '\0';
		}
		else
		{
			// this is the only body part, so output the MIME header
			MEMCOPY(szBegin, "MIME-Version: 1.0", sizeof("MIME-Version: 1.0"));
		}
...
#if _ATL_VER==0x0700
				//if (AtlMimeCharsetFromCodePage(szCharset, uCodePage, spMultiLanguage)) fix for 2003 [lama]
                if (AtlMimeCharsetFromCodePage(szCharset, uCodePage, spMultiLanguage, 0))
#else
