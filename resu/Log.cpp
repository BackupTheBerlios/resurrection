...
void AddModLogLine(LPCTSTR line, ...)
{
	ASSERT(line != NULL);

	va_list argptr;
	va_start(argptr, line);
	AddLogTextV(LOG_MOD, DLP_DEFAULT, line, argptr);
	va_end(argptr);
}
// [ionix]: ModLog added by Spe64*/
//Spe64 change --> ModLog
void AddModLogLine(bool bAddToStatusBar, LPCTSTR pszLine, ...)
{
	ASSERT(pszLine != NULL);

	va_list argptr;
	va_start(argptr, pszLine);
	AddLogTextV(LOG_MOD | (bAddToStatusBar ? LOG_STATUSBAR : 0), DLP_DEFAULT, pszLine, argptr);
	va_end(argptr);
}
