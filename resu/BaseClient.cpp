...
CUpDownClient::~CUpDownClient(){
	...
	m_DownloadBlocks_list.RemoveAll(); //Spanish [TPT] - Patch
	
	for (POSITION pos = m_RequestedFiles_list.GetHeadPosition();pos != 0;)
		delete m_RequestedFiles_list.GetNext(pos);
	m_RequestedFiles_list.RemoveAll(); //Spanish [TPT] - Patch
	...
	m_PendingBlocks_list.RemoveAll(); //Spanish [TPT] - Patch
...
{
        m_strModVersion.Empty(); //Spanish fix (Wizard/Xman)
...
// IP-to-Country +
	...
	{
		if(!IsBanned() && thePrefs.IsLeecherSecureLog())
			AddModLogLine(false, _T("Ban: (%s) tried to crash us -> banned!"), m_pszUsername); 
		CString msg;
...
void CUpDownClient::SendMuleInfoPacket(bool bAnswer){
	...
	data.WriteUInt32(7+1); // nr. of tags
...
// yonatan http start //////////////////////////////////////////////////////////////////////////
    ...
    // DonQ webcache -Support for tag ET_MOD_VERSION 0x55-
//>>> [lama] ModVersion
	CTag tagMod(ET_MOD_VERSION, MOD_VERSION);
	tagMod.WriteTagToFile(data);
	//<<< [lama] ModVersion
...
bool CUpDownClient::Disconnected(LPCTSTR pszReason, bool bFromSocket)
{
    ...
    //Spanish; MORPH - Removed by SIRoB, ensure that in any case the client will not stay in uploadqueue i.e. banned client 
...
//	}
	...
	switch(m_nUploadState){
		...
		case US_WAITCALLBACK:
			//MORPH START - Added by [ionix], Don't kill client if we are the only one complet source or it's a friend or it's a proxy.
			...
			//MORPH END   - Added by [ionix], Don't kill client if we are the only one complet source or it's a friend or it's a proxy.
		case US_ERROR:
	...
	switch(m_nDownloadState){
		...
		case DS_WAITCALLBACK:
			//MORPH START - Added by [ionix], Don't kill source if it's the only one complet source, it's a friend or a proxy
			if(m_bCompleteSource && reqfile->m_nCompleteSourcesCountLo == 1 || IsFriend() || !IsEd2kClient())
			{
				bDelete = true;
				break;
			}
			//MORPH END   - Added by [ionix], Don't kill source if it's a proxy
		case DS_ERROR:
			//KTS+ remove deadsource
			//theApp.clientlist->m_globDeadSourceList.AddDeadSource(this);
			//KTS- remove deadsource
...
void CUpDownClient::InitClientSoftwareVersion()
{
	...
	if (m_bEmuleProtocol || iHashType == SO_EMULE){
		...
		if (iLen > 0){
			MEMCOPY(m_strClientSoftware.GetBuffer(iLen), szSoftware, iLen*sizeof(TCHAR));
...
{
	 return theApp.clientlist->IsBannedClient(GetConnectIP()); //Xtreme Mod Bugfix
}
...
void CUpDownClient::AssertValid() const
{
	...
	(void)m_dwLastAskedTime; // bobo [Maella/sivka: -ReAsk SRCs after IP Change-]
