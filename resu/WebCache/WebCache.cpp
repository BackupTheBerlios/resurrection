...
//KTS-
#define MAX_PROXY_CONN (5*2)	//WiZaRd*2
...
// see if we can detect the webcache
	...
	if (FAILED(hr)) 
		throw _T("Failed to instantiate DOMDocument30 class.");
	...
	case 2:
	{
			if ( pXMLDom->load("") != VARIANT_TRUE)//removed sourceforge URL - lama
				throw _T("Your ISP was not found in the local database; loading the URL of xml data from the SF website failed.");
			...
			if (result != VARIANT_TRUE)
				throw _T("Failed loading xml data from the webcache-emule website.");
			break;
		}
	default:
