...
#define MAX_WCBLOCKLIST_SIZE (2000*5) //WiZaRd*5?
#define MAX_TIME_TO_KEEP_OHCB_IN_LIST MIN2MS(60) //this is currently only used for Stopped WC-Block-List
...
{
	if (!SINGLEProxyClient || !SINGLEProxyClient->m_pWCDownSocket) //WiZaRd - FiX?
...
POSITION pos = GetHeadPosition();
	while (pos)
	{
		...
		if (cur_block->m_uTime < GetTickCount()- MAX_TIME_TO_KEEP_OHCB_IN_LIST)
		{
			...
			delete cur_block;
			if (thePrefs.GetLogWebCacheEvents())
				AddDebugLogLine( false, _T("WebCache: Old Chunk Removed from Stopped List!!!!"));
		}
	}
/*
int i = 0;
