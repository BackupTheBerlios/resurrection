...
void CUpDownClient::DrawStatusBar(CDC* dc, LPCRECT rect, bool onlygreyrect, bool  bFlat) const
{
	...
	if (!onlygreyrect && reqfile && m_abyPartStatus)
	{
		...
		SAFE_DELETE_ARRAY(pcNextPendingBlks);
	}
...
void CUpDownClient::SendFileRequest()
{
	...
	bool bWebCache = (SupportsWebCache() && WebCacheInfoNeeded());
	//header+keylength+header+WebCacheNameLength
	const uint16 uiWebCacheBits = bWebCache?(8+sizeof(TCHAR)*thePrefs.webcacheName.GetLength() +8+16):0;
	CSafeMemFile dataFileReq(16+16+uiWebCacheBits);	// Superlexx - webcache - 64 extra bytes for webcache info data should be enough
	...
	if (SupportMultiPacket())
	{
		...
		//KTS- webcache
		if (thePrefs.GetDebugClientTCPLevel() > 0)
			DebugSend("OP__MultiPacket", this, (uchar*)reqfile->GetFileHash());
...
void CUpDownClient::ProcessFileInfo(CSafeMemFile* data, CPartFile* file)
{
	...
	//KTS- webcache
		{
		SAFE_DELETE_ARRAY(m_abyPartStatus);
...
//KTS-
			...
			if (nNewState == DS_NONE){
				SAFE_DELETE_ARRAY(m_abyPartStatus);//Spanish
				m_nPartCount = 0;
			}
...
void CUpDownClient::SendBlockRequests()
{
	...
	{
		const uint16 count = reqfile->GetNumberOfCurrentWebcacheConnectionsForThisFile();
		const uint16 allowed = reqfile->GetMaxNumberOfWebcacheConnectionsForThisFile();
		if (thePrefs.GetLogWebCacheEvents() && count) //JP log webcache events
			AddDebugLogLine(false, _T("WebCache: Proxy-Connections for this file: %u Allowed: %u"), count, allowed);

		//WiZaRd - resorted the checks to save CPU
		if( count < allowed
...
void CUpDownClient::ProcessBlockPacket(const uchar *packet, uint32 size, bool packed)
{
	...
	if(!IsDownloadingFromWebCache()) //don't count webcache download or it will screw up session ratio
...
//this is pretty bad behaviour and up2now a possible exploit!
//If a src tells he does not have a file we want but requests the SAME file himself
//then we will drop him if he wants that file...
//Pls note that this is *not* yet 100% secure
...
if(file && file == reqfile) //we speak about the same file
{ DebugLogWarning(_T("Dropped src: (%s) does not seem to have own reqfile!"), DbgGetClientInfo());
...
// WebCache //////////////////////////////////////////////////////////////////////////////
		...
		{
			DebugSend("OP__MultiFileReask", this, (uchar*)reqfile->GetFileHash());
