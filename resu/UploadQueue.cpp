...
//
//		//Aenarion: upload slot
//	if (thePrefs.GetUploadSlots())//Se l'opzione è abilitata
//	{
//		if(curUploadSlots >= thePrefs.GetMaxSlots())//Se il numero degli slot è >= agli slot massimi
//			return false;//Non accetare il client
//	}//Aenarion: upload slot
//
...
bool CUploadQueue::AcceptNewClient(uint32 curUploadSlots){
	...
	if( curUploadSlots >= /*MAX_UP_CLIENTS_ALLOWED*/12 )//se gli slot sono maggiori di 12 
		return false;//Non accetare altri client

	if( curUploadSlots <  thePrefs.UpDataratePerClient )//se gli slot sono minori di quelli che voglio io
		return true;//Accetta un altro client

	return false;//Se le 2 condizioni non si verificano non acettare altri client
}
//end - modified - up datarate
...
// WebCache end/////////////////////////////////////////////////////////////////////////////////////
	...
	// statistic values
	uploadReqfile->statistic.AddRequest();// thanx wizzi for helping 
