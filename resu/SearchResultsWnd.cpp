	...
	if (!pParams->strBooleanExpr.IsEmpty())
	{
		...
		if (!pParams->bUnicode)
		{
			...
			if (!IsValidEd2kStringA(strACP)){
				...
				throw new CMsgBoxException(strError/*, MB_ICONWARNING | MB_HELP, eMule_FAQ_Search - HID_BASE_PROMPT*/);//removed help [lama]
			}
		}
...
bool CSearchResultsWnd::DoNewKadSearch(SSearchParams* pParams)
{
	...
	{
		delete pSearchTermsData;
		throw new CMsgBoxException(strException/*, MB_ICONWARNING | MB_HELP, eMule_FAQ_Search - HID_BASE_PROMPT*/);//removed help [lama]
	}
