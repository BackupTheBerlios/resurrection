...
#include <share.h>
//KTS+ webcache
#include "secrunasuser.h" // yonatan - moved up... 
//KTS- webcache
#include "emule.h"
...
// MOD Note: Do not change this part - Merkur
	...
	ASSERT( VERSION_UPDATE + 'a' <= 'f' );//[lama] ModVersion
#ifdef _DEBUG
	m_strCurVersionLong.Format(_T("%u.%u%c.%u | %s"), VERSION_MJR, VERSION_MIN, _T('a') + VERSION_UPDATE, VERSION_BUILD, MOD_VERSION); //>>> [lama] ModVersion
#else
	m_strCurVersionLong.Format(_T("%u.%u%c | %s"), VERSION_MJR, VERSION_MIN, _T('a') + VERSION_UPDATE, MOD_VERSION); //>>> [lama] ModVersion
#endif
	...
	strTmp.Format(_T("0x%u%c"), m_dwProductVersionMS, _T('A') + VERSION_UPDATE);//[lama] ModVersion
...
#error "You are using an MFC version which may require a special version of the above function!"
    ...
    theAntiLeechClass.Init(); //>>> AntiLeech Class added by lama
	thePrefs.Init();
	theStats.Init();
	// >>> [iONiX] - WiZaRd - Multiple Instances added by lama
	if (ProcessCommandline())
...
thePerfLog.Startup();
	...
	// >>> [iONiX] - WiZaRd - Multiple Instances added by lama
	if (m_reset_app){
		CString m_eMuleExePath;
		m_eMuleExePath.Format(_T("%s%s.exe"), thePrefs.GetAppDir(), m_pszExeName);
		ShellOpenFile(m_eMuleExePath);
	}
	// <<< [iONiX] - WiZaRd - Multiple Instances added by lama
...
bool CemuleApp::ProcessCommandline()
{
 ...
 if ((maininst) && !((thePrefs.GetAllowMultipleInstances() && cmdInfo.m_strFileName.GetLength()==0) || bIgnoreRunningInstances)){ // <<< [iONiX] - WiZaRd - Multiple Instances added by lama
...
BOOL CALLBACK CemuleApp::SearchEmuleWindow(HWND hWnd, LPARAM lParam){
	...
	if(dwMsgResult == UWM_ARE_YOU_EMULE)
		// >>> [iONiX] - WiZaRd - Multiple Instances added by lama
		if (GetWindowLong(hWnd, GWL_USERDATA) == MAGIC)
	{ 
