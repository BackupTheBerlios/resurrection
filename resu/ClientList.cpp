		...
		for (pos1 = list.GetHeadPosition();( pos2 = pos1 ) != NULL;){
			...
			if ((pCurClient->GetUploadState() == US_NONE || pCurClient->GetUploadState() == US_BANNED && !pCurClient->IsBanned())
				...
				&& pCurClient->socket == NULL
                                && pCurClient->GetRequestFile() == NULL                                         // it's possible that a client is added to queue, later is dropped because of queue-full.
...
void CClientList::TrigReaskForDownload(){
	...
	for(POSITION pos = list.GetHeadPosition(); pos;){
		...
		if( cur_client->GetDownloadState() == DS_NONEEDEDPARTS )
			...
			cur_client->SetLastAskedTime(thePrefs.uReAskFileSRC-600000);
	}	
}
// Maella end
