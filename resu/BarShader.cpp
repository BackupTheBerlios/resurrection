...
CBarShader::CBarShader(uint32 height, uint32 width): m_Spans(100) //Spanish [TPT] Xman. Maella -Code Improvement (CPU load)- 
{
	m_iWidth = width;
	m_iHeight = height;
	m_uFileSize = 0; // [ionix] - TPT: Patch	
	m_Spans.SetAt(0, 0);	// SLUGFILLER: speedBarShader
	m_Modifiers = NULL;
	m_bIsPreview=false;
	SetFileSize(1); // [ionix] - TPT: Patch	
}
