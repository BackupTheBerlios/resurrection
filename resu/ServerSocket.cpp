	...
	try{
		switch(opcode){
			...
			case OP_IDCHANGE:{
				...
				// added by bobo [Maella/sivka: -ReAsk SRCs after IP Change-]
				...
				// about the new IP.
				if( thePrefs.uLastKnownID != serverconnect->clientid ){
					// [ionix] - WiZaRd - quick start after ip change
					if(thePrefs.GetQuickStart() && thePrefs.GetQuickStartAfterIPChange())
						{
						thePrefs.quickflag = thePrefs.quickflags = false;
						}
					// [ionix] - WiZaRd - quick start after ip change
					AddModLogLine(false, _T("CLIENT-ID changed from %s=%u to %s=%u"),
					((thePrefs.uLastKnownID<16777216)?_T("LowID"):_T("HighID")), thePrefs.uLastKnownID,
					((serverconnect->clientid<16777216)?_T("LowID"):_T("HighID")), serverconnect->clientid);
					if( serverconnect->clientid >= 16777216 ){
						AddModLogLine(false, _T("CLIENT-IP changed from IP=%s to IP=%s"), ipstr(thePrefs.uLastKnownIP), ipstr(serverconnect->clientid));
						if( thePrefs.bReAskSRCAfterIPChange && thePrefs.uLastKnownIP != 0 ){
							theApp.clientlist->TrigReaskForDownload();
							AddModLogLine(false, _T("CLIENT-IP was changed, all sources will be reasked inside the next %u mins!"), thePrefs.GetQuickStartMinutes());
						}
						else if( thePrefs.uLastKnownIP != 0 )
							AddModLogLine(false, _T("Feature \"ReAsk SRCs after IP Change\" --> DISABLED!!!"));
						thePrefs.uLastKnownIP = serverconnect->clientid; // Keep track of a change of the global IP
					}
					else 
						AddModLogLine(false, _T("Feature \"ReAsk SRCs after IP Change\" --> DONT WORK WITH LOWID!!!"));
					thePrefs.uLastKnownID = serverconnect->clientid; // Keep track of a change of the global ID
				}
				// [Maella/sivka: -ReAsk SRCs after IP Change-] End
