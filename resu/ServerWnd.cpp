...
#include "Log.h"
#include "UserMsgs.h"
#include "WebCache/WebCache.h" //JP webcache version shown in serverlog at start
	...
	: CResizableDialog(CServerWnd::IDD, pParent)
{
	...
	modlog = new CHTRichEditCtrl; // [ionix] ModLog Added by Spe64
...
CServerWnd::~CServerWnd()
{
	...
	delete debuglog;
	delete logbox;
	delete modlog; // [ionix] ModLog Added bt Spe64
	delete servermsgbox;
...
#define	LOG_PANE_RICHEDIT_STYLES WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_MULTILINE | ES_READONLY | ES_NOHIDESEL
	...
	GetDlgItem(IDC_MODLOG)->GetWindowRect(rect);
	GetDlgItem(IDC_MODLOG)->DestroyWindow();
	::MapWindowPoints(NULL, m_hWnd, (LPPOINT)&rect, 2);
	if (modlog->Create(LOG_PANE_RICHEDIT_STYLES, rect, this, IDC_MODLOG)){
		modlog->SetProfileSkinKey(_T("Log"));
		modlog->ModifyStyleEx(0, WS_EX_STATICEDGE, SWP_FRAMECHANGED);
		modlog->SendMessage(EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN, MAKELONG(3, 3));
		if (theApp.m_fontLog.m_hObject)
			modlog->SetFont(&theApp.m_fontLog);
		modlog->ApplySkin();
		modlog->SetTitle(_T("ReSuRReCTioN-Log"));
		modlog->SetAutoURLDetect(FALSE);
	}
	// [ionix] ModLog Added by Spe64
	...
	// [ionix] ModLog Added by Spe64
	name = _T("ReSuRReCTioN-Log"); // <--- > change name here
	newitem.mask = TCIF_TEXT|TCIF_IMAGE;
	newitem.pszText = const_cast<LPTSTR>((LPCTSTR)name);
	...
	modlog->ShowWindow(SW_HIDE); // [ionix] ModLog Added by Spe64
...
void CServerWnd::SetAllIcons()
{
	...
	iml.Add(CTempIconLoader(_T("TRAYCONNECTED"))); // [ionix] ModLog Added by Spe64 and change icon for test
...
void CServerWnd::OnBnClickedResetLog()
{
	...
	if (cur_sel == PaneModLog) { // [ionix] ModLog
...
//Added by Spe64 ---> Modlog
	if (cur_sel == PaneModLog) { // [ionix] ModLog
		...
		logbox->ShowWindow(SW_HIDE);
		debuglog->ShowWindow(SW_HIDE);
		modlog->ShowWindow(SW_SHOW);
		if (modlog->IsAutoScroll() && (StatusSelector.GetItemState(cur_sel, TCIS_HIGHLIGHTED) & TCIS_HIGHLIGHTED))
			modlog->ScrollToLastLine(true);
		modlog->Invalidate();
		StatusSelector.HighlightItem(cur_sel, FALSE);
	}
//Spe64-
...
//KTS+ Display User Hash
	...
	m_MyInfo << GetResString(IDS_PW_GENERAL) <<   _T("\r\n");
	m_MyInfo.SetSelectionCharFormat(m_cfDef);

	m_MyInfo << GetResString(IDS_PW_NICK) <<   _T(":\t");
	buffer.Format(  _T("%s"),thePrefs.GetUserNick());
	m_MyInfo << buffer <<   _T("\r\n");

	m_MyInfo << GetResString(IDS_CD_UHASH) <<   _T("\t");
	buffer.Format(  _T("%s"),(LPCTSTR)(md4str((uchar*)thePrefs.GetUserHash())));
	buffer2.Format(  _T("%s"),thePrefs.GetOldUserHash());
	if (buffer != buffer2)
		m_MyInfo.SetSelectionCharFormat(m_cfRed);
	m_MyInfo << buffer <<   _T("\r\n");
	m_MyInfo.SetSelectionCharFormat(m_cfDef);
	m_MyInfo <<   _T("\r\n");
	//KTS- Display User Hash
...
void CServerWnd::DoResize(int delta)
{
	...
	CSplitterControl::ChangeHeight(GetDlgItem(IDC_MODLOG), -delta, CW_BOTTOMALIGN); //Spe64 ModLog
...
void CServerWnd::InitSplitter()
{
	...
	//KTS+ > Spe64 Added ModLog
	modlog->GetWindowRect(rcDlgItem);
	ScreenToClient(rcDlgItem);
	rcDlgItem.top=splitpos+35;
	rcDlgItem.bottom = rcWnd.bottom-12;
	modlog->MoveWindow(rcDlgItem);
	//KTS- > Added by Spe64
...
void CServerWnd::ReattachAnchors() {
	...
	RemoveAnchor(*modlog); //Added by Spe64 ModLog
	...
	AddAnchor(*modlog,  CSize(0, thePrefs.GetSplitterbarPositionServer()), BOTTOM_RIGHT); //Added by Spe64 ModLog
