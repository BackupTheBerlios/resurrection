...
//	Auto-refresh code
	...
	//Out.Replace(_T("[Connect]"), _GetPlainResString(IDS_IRC_CONNECT)); removed irc [lama]
	...
	if (Kademlia::CKademlia::isConnected()) {
		if (Kademlia::CKademlia::isFirewalled()) {
			...
			HTTPConText.AppendFormat(_T(", <a href=\"?ses=%s&w=kad&c=disconnect\">%s</a>)"), sSession /*, GetResString(IDS_IRC_DISCONNECT)*/ ); //changed for remove irc [lama]
		} else {
		...
		if (Kademlia::CKademlia::isRunning()) {
			...
			HTTPConText.AppendFormat(_T(" (<a href=\"?ses=%s&w=kad&c=connect\">%s</a>)"),  sSession /*, GetResString(IDS_IRC_CONNECT)*/ ); //changed for remove irc [lama]
		}
	}
...
CString CWebServer::_CreateTransferList(CString Out, CWebServer *pThis, ThreadData* Data, void* _FilesArray, void* _UploadArray, bool bAdmin) {
	...
	if(pThis->m_Params.bShowUploadQueueFriend)
	{
		...
		//Out.Replace(_T("[UploadQueueFriendList]"), _GetPlainResString(IDS_IRC_ADDTOFRIENDLIST)); removed irc [lama]
...
CString CWebServer::_GetKadDlg(ThreadData Data)
{
	...
	if (Kademlia::CKademlia::isConnected()) {
		if (Kademlia::CKademlia::isFirewalled()) {
			...
			buffer.AppendFormat(_T("<br><a href=\"?ses=%s&w=kad&c=disconnect\">%s</a>"), sSession /*, GetResString(IDS_IRC_DISCONNECT)*/ );//remove irc [lama]
		} else {

			Out.Replace(_T("[KADSTATUS]"), GetResString(IDS_CONNECTED));
			buffer.Format(_T("<a href=\"?ses=%s&w=kad&c=disconnect\">%s</a>"),  sSession /*, GetResString(IDS_IRC_DISCONNECT) */);//remove irc [lama]
		}
	}
	else {
		if (Kademlia::CKademlia::isRunning()) {
			...
			buffer.Format(_T("<a href=\"?ses=%s&w=kad&c=connect\">%s</a>"),  sSession /*, GetResString(IDS_IRC_CONNECT)*/ );//remove irc [lama]
		}
	}
