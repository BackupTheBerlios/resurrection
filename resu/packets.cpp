...
Packet::Packet(const CStringA& str, uint8 ucProtocol, uint8 ucOpcode){
	...
	pBuffer = completebuffer+6;
	MEMCOPY(pBuffer,(LPCSTR)str,size);
	tempbuffer = 0;
	opcode = ucOpcode;
	prot = ucProtocol;
}
...
CRawPacket::CRawPacket(const CStringA& rstr)
{
	...
	MEMCOPY(pBuffer, (LPCSTR)rstr, size);
}
...
CTag::CTag(const CTag& rTag)
{
	...
	if (rTag.IsStr())
		...
		MEMCOPY(m_pData, rTag.GetBlob(), rTag.GetBlobSize());
	}
	else{
		ASSERT(0);
		m_uVal = 0;
