...
//==> Toolbar [shadow2004]
	...
	m_co_TransferBtn.Set_TextPos(trect5);
	m_co_SearchBtn.Set_TextPos(trect5);
	m_co_SharedBtn.Set_TextPos(trect6);
	m_co_MessagesBtn.Set_TextPos(trect5);//button zu kurz *lol
	m_co_StatisticBtn.Set_TextPos(trect5);
	m_co_PreferencesBtn.Set_TextPos(trect5);
	m_co_ToolsBtn.Set_TextPos(trect5);
		...
		switch(theApp.emuledlg->status){
			...
			case 4:{
				...
				if (!bError) // show the success msg, only if we had no serious error
					...
					AddModLogLine(false,_T("Winsock: Version %d.%d"), HIBYTE( theApp.m_wsaData.wVersion ),LOBYTE(theApp.m_wsaData.wVersion));
					if (theApp.m_wsaData.iMaxSockets!=0)
						AddModLogLine(false,_T("Winsock: max. sockets %d"), theApp.m_wsaData.iMaxSockets);
					else
						AddModLogLine(false,_T("Winsock: unlimited sockets"));
					AddModLogLine(true, GetResString(IDS_MAIN_READY),theApp.m_strCurVersionLong); //MORPH - Added by milobac, Translation version info
				}//Spe64 added eWombat [WINSOCK2]
...
//Added by Spe64 ModLog
void CemuleDlg::ResetModLog() // [ionix] ModLog
{
	serverwnd->modlog->Reset();
}
//Spe64-
void CemuleDlg::AddLogText(UINT uFlags, LPCTSTR pszText)
{
	...
	if (iLen >= 0)
	{
		if (!(uFlags & (LOG_DEBUG | LOG_MOD /* [ionix] ModLog */))) //Spe 64 Added ModLog
		{
	...
	//Added by Spe64 ModLog
	if (uFlags & LOG_MOD) { // [ionix] ModLog
			serverwnd->modlog->AddTyped(temp, iLen, uFlags);
			if (IsWindow(serverwnd->StatusSelector) && serverwnd->StatusSelector.GetCurSel() != CServerWnd::PaneModLog) // [ionix] ModLog
				serverwnd->StatusSelector.HighlightItem(CServerWnd::PaneModLog, TRUE); // [ionix] ModLog
		}
	//Spe64-
...
// [ionix] ModLog Added by Spe64
CString CemuleDlg::GetLastModLogEntry()
{
	return serverwnd->modlog->GetLastLogEntry();
}

CString CemuleDlg::GetAllModLogEntries()
{
	return serverwnd->modlog->GetAllLogEntries();
}
// [ionix] ModLog Added by Spe64
...
void CemuleDlg::ShowTransferRate(bool bForceAll)
{
	...
	if (TrayIsVisible() || bForceAll)
	{
		...
		/*
		if (theApp.IsConnected())
		_sntprintf(buffer2, ARRSIZE(buffer2), _T("eMule (%s)\r\n%s"), GetResString(IDS_CONNECTED), strTransferRate);
		else
		_sntprintf(buffer2, ARRSIZE(buffer2), _T("eMule (%s)\r\n%s"), GetResString(IDS_DISCONNECTED), strTransferRate);
		*/

		//Should separate for new and old tooltip support... but I'm lazy wink.gif
		if (theApp.IsConnected()) 
			_sntprintf(buffer2,ARRSIZE(buffer2),_T("eMule %s (%s)\r\n%s"), theApp.m_strCurVersionLong, GetResString(IDS_CONNECTED), strTransferRate);
		else
			_sntprintf(buffer2,ARRSIZE(buffer2),_T("eMule %s (%s)\r\n%s"), theApp.m_strCurVersionLong, GetResString(IDS_DISCONNECTED), strTransferRate);

		// Not needed, will be truncated in the setting function at the appropriate length
		//buffer2[63] = _T('\0');
		// </CB Mod : Tooltip>
...
{
	// >>> [iONiX] - WiZaRd - Multiple Instances added by lama
	//if (!CanClose())
	if (!theApp.m_reset_app && !CanClose())
		return;
	// <<< [iONiX] - WiZaRd - Multiple Instances added by lama
	...
	//theApp.scheduler->RestoreOriginals(); removed scheduler [lama]
	thePrefs.Save();
	thePerfLog.Shutdown();
	...
	if (WSAGETASYNCERROR(lParam) == 0)
	{
		...
		if (iBufLen >= sizeof(HOSTENT))
		{
			...
			if (pHost->h_length == 4 && pHost->h_addr_list && pHost->h_addr_list[0])
			{
				...
				uint8 abyCurVer[4] = { VERSION_BUILD + 1, VERSION_UPDATE, VERSION_MIN, 0}; //[lama] ModVersion
				dwResult &= 0x00FFFFFF;
