...
//		nFreeDiskSpace.QuadPart=0;
	...
	else 
	{
		...
		if (p)
		{
			MEMCOPY(cDrive, pDirectory, (p - pDirectory)*sizeof(TCHAR));
...
//Start - Added by SiRoB, Used in Fake and ipfilter updater
...
//End   - Added by SiRoB, Used in Fake and ipfilter updater
...
bool SupportExtendedToolTips() {
	if(GetDllVersion(TEXT("comctl32.dll")) >= PACKVERSION(5,0))
		return true;
	else
		return false;
}
