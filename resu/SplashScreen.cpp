...
BOOL CSplashScreen::OnInitDialog()
{
	...
	if (m_imgSplash.GetSafeHandle())
	{
		...
		if (m_imgSplash.GetBitmap(&bmp) > 0)
		{
			...
			//>>> 4 Lama only - Calc new coordinates 
			const int width = GetSystemMetrics(SM_CXSCREEN); 
			const int height = GetSystemMetrics(SM_CYSCREEN); 

			wp.rcNormalPosition.left = (width-bmp.bmWidth)/2; 
			wp.rcNormalPosition.right = wp.rcNormalPosition.left + bmp.bmWidth;

			wp.rcNormalPosition.top = (height-bmp.bmHeight)/2; 
...
void CSplashScreen::OnPaint() 
{
	...
	if (m_imgSplash.GetSafeHandle())
	{
		...
		if (dcMem.CreateCompatibleDC(&dc))
		{
			...
			CRect rc(BM.bmWidth - 70, 2, BM.bmWidth - 2, 2 + htext);
			...
			COLORREF oldclr = dc.SetTextColor(RGB(255,128,128));
			int iOMode = dc.SetBkMode(TRANSPARENT);
			CFont font;
			font.CreateFontIndirect(&lf);
			CFont* pOldFont = dc.SelectObject(&font);
			//CString strAppVersion(MOD_NUMBER_VERSION); //kannst du nachher dranhängen, wenns überhaupt gewünscht ist...
