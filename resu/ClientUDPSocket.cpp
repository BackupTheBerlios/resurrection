		...
		case OP_REASKCALLBACKUDP:
		{
			...
			if( buddy )
			{
				...
				if (!md4cmp(packet, buddy->GetBuddyID()))
				{
					...
					MEMCOPY(response->pBuffer, packet+10, size-10);
...
//				AddDebugLogLine( false, _T("Recv UDP-WCB: %d"), *id );
			...
			if( sender ) 
			{
				...
				{
					if (thePrefs.GetLogWebCacheEvents())
							AddDebugLogLine( false, _T("WebCache: Received WCBlock - UDP") );
		...
		case OP_MULTI_FILE_REASK:
			{
				...
				if (sender)
				{
					...
					//sender->ProcessExtendedInfo(&data_in, reqfile);
//>>> WiZaRd
					//Clients will send partstatus for the first file, too...
					if (sender->GetUDPVersion() > 3)
					{
						sender->ProcessExtendedInfo(&data_in, reqfile);
					}

					//... and they will send the src count!
					else if (sender->GetUDPVersion() > 2)
					{
						uint16 nCompleteCountLast= sender->GetUpCompleteSourcesCount();
						uint16 nCompleteCountNew = data_in.ReadUInt16();
					...
					//now we can go on and add the additional files...
//<<< WiZaRd 
					try
					{
